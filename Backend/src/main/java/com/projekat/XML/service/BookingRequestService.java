...
public class BookingRequestService {
    ...
    public void makeRequests(List<ItemInCartDTO> listaZahteva) throws DatatypeConfigurationException {
        ...
        for (Long id : vlasnici) {
            ...
            if (!seprate.isEmpty()) {
                ...
                for (ItemInCartDTO ItemInCartDTO2 : seprate) {
                    ...
                    bookingClient.saveSoapBooking(new BookingRequest(ItemInCartDTO2.getAdvertisement().getPostedBy().getId(), getLogedUserId(),
                            lastGroupId, RequestStates.PENDING, ItemInCartDTO2.getAdvertisement(),
            ...
            if (!together.isEmpty()) {
                ...
                for (ItemInCartDTO ItemInCartDTO : together) {
                    ...
                    bookingClient.saveSoapBooking(new BookingRequest(ItemInCartDTO.getAdvertisement().getPostedBy().getId(), getLogedUserId(),
                            lastGroupId, RequestStates.PENDING, ItemInCartDTO.getAdvertisement(),
                            ItemInCartDTO.isTogether(), ItemInCartDTO.getTimeFrom(), ItemInCartDTO.getTimeTo()));
